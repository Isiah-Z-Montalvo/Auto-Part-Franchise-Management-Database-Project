...
// Rest of the functions definition go in here
   ...
   public static void placeOrder(Retail esql) {
      try{
	    ...
	    String queryLs = String.format("SELECT * FROM Orders ORDER BY orderTime DESC LIMIT 1");
	    esql.executeQueryAndPrintResult(queryLs);
   ...
   public static void viewRecentOrders(Retail esql) {
	...
	String queryC = String.format("SELECT o.storeID, s.name, o.productName, o.unitsOrdered, o.orderTime FROM Store s, Orders o WHERE o.storeID = s.storeID AND o.customerID = '%s' ORDER BY o.orderTime DESC LIMIT 5", userID);
	int userNum = esql.executeQueryAndPrintResult(queryC);
	} else if (userType.equals("manager") || userType.equals("admin")){
			if (userType.equals("admin")){
			String queryID = String.format("SELECT userID FROM Users WHERE type = 'manager'");
			List<List<String>> mids = esql.executeQueryAndReturnResult(queryID);
			List<String> mngID = mids.stream().flatMap(List::stream).collect(Collectors.toList());
			System.out.print(mngID);
			System.out.print("\nWhich ManagerID would you like to proceed as? ");
			userID = in.readLine();
			while (mngID.contains(userID) == false){
				System.out.print("That UserID is not a manager! Please enter a ManagerID: ");
				userID = in.readLine();
			}
		}
	String query2 = String.format("SELECT DISTINCT Orders.orderNumber, Users.name, Orders.storeID, Orders.productName, Orders.orderTime FROM Orders JOIN Users ON Orders.customerID=Users.userID JOIN Product ON Orders.storeID=Product.storeID JOIN Store ON Product.storeID=Store.storeID WHERE managerID = '%s'", userID);
   ...
   public static void updateProduct(Retail esql) {
      ...
      if(userType.equals("manager") || userType.equals("admin")){
		if (userType.equals("admin")){
         ...
         System.out.println("You're not a manager or admin.");
      }
      } catch(Exception e){
         System.err.println(e.getMessage());
      }
   ...
   public static void viewRecentUpdates(Retail esql) {
      ...
      if(userType.equals("manager") || userType.equals("admin")){
		if (userType.equals("admin")){
         ...
         String query1 = String.format("SELECT * FROM ProductUpdates WHERE managerID = '%s' ORDER BY updatedOn DESC LIMIT 5", userID);
         int userNum = esql.executeQueryAndPrintResult(query1);
      }
      else{
         System.out.println("You're not a manager or an admin.");
      }
      }catch(Exception e){
           System.err.println (e.getMessage());
      }
   ...
   public static void viewPopularProducts(Retail esql){
	...
	if (userType.equals("manager") || userType.equals("admin")){
		...
		String query1 = String.format("SELECT Orders.productName, SUM(Orders.unitsOrdered) FROM Orders JOIN Product ON Orders.storeID=Product.storeID AND Orders.productName=Product.productName JOIN Store ON Product.storeID=Store.storeID WHERE managerID = '%s' GROUP BY Orders.productName ORDER BY SUM(unitsOrdered) DESC LIMIT 5", userID);
   ...
   public static void viewPopularCustomers(Retail esql) {
	...
	if (userType.equals("manager") || userType.equals("admin")){
		...
		String query1 = String.format("SELECT Users.userID, Users.name, Users.type, COUNT(Orders.orderNumber) FROM Users JOIN Orders ON Users.userID=Orders.customerID JOIN Product ON Orders.storeID=Product.storeID JOIN Store ON Product.storeID=Store.storeID WHERE managerID = '%s' GROUP BY Users.userID, Users.name, Users.type ORDER BY COUNT(Orders.orderNumber) DESC LIMIT 5", userID);
		esql.executeQueryAndPrintResult(query1);		
	}
	else{
		System.out.println("You're not a manager or an admin.");
   ...
   public static void placeProductSupplyRequests(Retail esql) {
	...
	if (userType.equals("manager") || userType.equals("admin")){
		...
		if (userType.equals("manager")){
			String queryID = String.format("SELECT storeID FROM Store WHERE managerID = '%s'", userID);
			List<List<String>> sids = esql.executeQueryAndReturnResult(queryID);
			List<String> stores = sids.stream().flatMap(List::stream).collect(Collectors.toList());
			System.out.print(stores);
			System.out.print("\nWhich Store?: ");
			storeID = in.readLine();
			while (stores.contains(storeID) == false){
				System.out.print("You don't manage that store! Please enter a valid StoreID: ");
				storeID = in.readLine();
			}		
		}
		else{
		...
		System.out.print("Product Name: ");
		String productName = in.readLine();
		System.out.print("Number of Units Needed: ");
		String unitsRequested = in.readLine();
		System.out.print("Warehouse ID: ");
		String warehouseID = in.readLine();

		String query1 = String.format("INSERT INTO ProductSupplyRequests(managerID, warehouseID, storeID, productName, unitsRequested) VALUES ('%s', '%s', '%s', '%s', '%s')", userID, warehouseID, storeID, productName, unitsRequested);
		esql.executeUpdate(query1);
		String queryLs = String.format("SELECT * FROM ProductSupplyRequests ORDER BY requestNumber DESC LIMIT 1");
		esql.executeQueryAndPrintResult(queryLs);
		String query2 = String.format("UPDATE Product SET numberOfUnits = numberOfUnits + '%s' WHERE storeID = '%s' AND productName = '%s'", unitsRequested, storeID, productName);
